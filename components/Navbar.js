...
function Navbar({
  ...
  return (
    <>
      <div className=" flex py-4  border-b lg:justify-start justify-center align-middle    items-center sticky top-0 z-20 opacity-100 bg-white  ">
        <div className=" flex  sm:ml-20  ml-7 items-center align-middle  ">
          <Link href={"/"}>
            <div className="flex   ">
              <Image src={"/images/bizhub.png"} width={110} height={2}></Image>
            </div>
          </Link>
        </div>

        <div className="cart lg:flex hidden   items-center absolute sm:right-16 right-2 mx-2  align-middle">
          {!myUser.token && (
            <div>
              <Link href={"/login"}>
                <span className="md:font-bold md:text-lg font-serif  mx-2 underline  ">
                  Join Now
                </span>{" "}
              </Link>
            </div>
          ...
          {myUser.token && (
            <div className="sm:block hidden">
              <Link href={"/createPost"}>
                <div className="md:font-semibold md:text-base font-serif items-center  px-3.5 py-1 mx-2 bg-white border-slate-900 border-2 rounded-full text-green-600 ">
                  Create Post +
                </div>{" "}
              </Link>
            </div>
          ...
          >
            {togel && (
              <div
                ...
                className="z-10 absolute top-8 cursor-pointer right-4 bg-white divide-y divide-gray-100 rounded-sm shadow w-48 duration-500 transition-opacity  delay-150"
              >
                <Link href={"/myAccount"}>
        ...
        <div class="bottom fixed bottom-0 bg-slate-50 w-full h-12 z-20 lg:hidden flex ">
          ...
          <div class="box flex justify-center items-center w-1/4 cursor-pointer">
            <span class="relative">
              {myUser.token ? (
                <Link href={"/createPost"}>
                  <span class="absolute -top-12 z-20 -right-6 h-12 w-12 text-lg rounded-full bg-green-600/90 border border-green-600 text-white flex justify-center items-center items cursor-pointer">
                    <span>
                      <AiOutlinePlus />
                    </span>
                  </span>
                ...
                <Link href={"/login"}>
                  <span class="absolute -top-12 -right-6 h-12 w-12 z-20 text-lg rounded-full bg-green-600/90 border border-green-600 text-white flex justify-center items-center items cursor-pointer">
                    <span>
                      <AiOutlinePlus />
                    </span>
                  </span>
